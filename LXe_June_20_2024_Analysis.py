...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 12 13:54:56 2024
...
import pickle
from LXe_June_2024_Analysis import JuneCampaign

files = ['Run_1718905375.hdf5', 'Run_1718906396.hdf5', 'Run_1718907788.hdf5', 'Run_1718908742.hdf5','Run_1718909887.hdf5','Run_1718911129.hdf5','Run_1718911950.hdf5','Run_1718912614.hdf5','Run_1718913535.hdf5']

upperlim = [0.15,0.04,0.12,0.05,0.035,0.25,0.25,0.1375,0.05]

proms = [0.005,0.005,0.005,0.005,0.005,0.005,0.005,0.005,0.005]



#%%
run_spe_solicited = RunInfo(['Run_1718905077.hdf5'],
...
for i in range(len(runs)):
    ...
    wp.process(do_spe=True, do_alpha=False)
    campaign.append(wp)
curr_campaign = campaign

spe_current = SPE_data(curr_campaign, JuneCampaign,invC, invC_err, filtered=False)
# %%
spe_current.plot_spe(in_ov=False, absolute=False)

# %%
spe_CA = SPE_data(curr_campaign, invC, invC_err, filtered=True)
spe_CA.plot_CA(
    out_file='/Users/yaozhiyan/Documents/Research/nEXO/OneDrive_1_7-11-2024/LXe_CA.csv')